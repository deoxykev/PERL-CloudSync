...
#  	my $loginsDBM = pDrive::DBM->new(pDrive::Config->DBM_LOGIN_FILE);
	...
	if ($token eq '' or  $refreshToken  eq ''){
		my $code;
		my  $URL = 'https://accounts.google.com/o/oauth2/auth?scope=https://picasaweb.google.com/data/&redirect_uri=urn:ietf:wg:oauth:2.0:oob&response_type=code&client_id='.pDrive::Config->CLIENT_ID;
...
sub uploadFile(*$$){
	...
	print STDOUT $file . "\n";
  	...
  	my $uploadURL = 'http://picasaweb.google.com/data/upload/resumable/photos/create-session/feed/api/user/default/albumid/default';#https://picasaweb.google.com/data/feed/api/user/default/albumid/default';#= $self->{_serviceapi}->createFile('https://www.googleapis.com/upload/drive/v2/files?fields=id&convert=false&uploadType=resumable',$fileSize,$fileName,$filetype, $folder);
  	...
  	my $retrycount=0;
    	...
    	while ($status eq '0' and $retrycount < 5){
			$status = $self->{_serviceapi}->uploadFile($uploadURL,$file,$filetype);
